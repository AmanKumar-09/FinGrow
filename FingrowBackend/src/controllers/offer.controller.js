...
const getOffersByAmount = asynchandler(async (req, res) => {
    ...
    if (!offers || offers.length === 0) {
        return res.status(200).json(new Apiresponse(200, true, "No loan offers found within the specified amount", []));
    }

    // Return the found offers
    return res.status(200).json(new Apiresponse(200, true, "Loan offers retrieved successfully", offers));
});
