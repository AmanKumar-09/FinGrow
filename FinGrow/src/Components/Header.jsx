...
const Header = ({ toggleProfile, isProfileOpen }) => {
  ...
  // Initial height matching original
  const INITIAL_HEIGHT = 128; // Base height in pixels
  ...
  // Calculate dynamic height only when scrolled past a threshold
      ...
      <nav className="flex list-none gap-12 items-center justify-center text-xl pr-12">
        {links.map((link, index) => (
          <NavLink
            ...
            onClick={() => window.scrollTo(0, 0)} // Reset scroll to top on click
          >
            {link.name}
          </NavLink>
        ))}
