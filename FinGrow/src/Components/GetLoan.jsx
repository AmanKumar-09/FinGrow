...
function GetLoan() {
  ...
  const formattedDate = repaymentDate.toLocaleDateString("en-GB"); // DD/MM/YYYY format

  // Handle phone number input
  const handlePhoneChange = (e) => {
    let value = e.target.value.replace(/\D/g, ""); // Remove non-numeric characters
    if (value.startsWith("91")) {
      value = value.slice(2); // Remove extra "91" if already present
    }
    setPhone("91" + value.slice(0, 10)); // Ensure it always starts with 91 and is 10 digits long
  };

  return (
    <div className="flex justify-center items-center min-h-screen bg-gray-100 p-5">
      <div className="flex flex-col md:flex-row bg-white shadow-2xl rounded-3xl overflow-hidden w-[900px] h-auto">
        {/* Left Side - Loan Info */}
        ...
        <div className="bg-blue-600 p-8 md:w-1/3 flex flex-col justify-center items-center rounded-r-3xl">
          <input
            ...
            className="w-full border border-gray-300 rounded-lg p-3 text-center text-lg outline-none focus:ring-2 focus:ring-white"
          />
          <p className="text-white text-xs text-center mt-2">
            Enter your mobile number
          </p>

          <button className="mt-4 bg-white text-blue-600 font-bold py-3 px-6 rounded-lg shadow-lg hover:bg-gray-100 transition duration-200">
            GET CASH TODAY
          </button>
